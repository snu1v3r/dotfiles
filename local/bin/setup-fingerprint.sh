...
if [[ "--remove" == "$1" ]]; then
  ...
  yay -Rns --noconfirm fprintd libfprint-ft9366
  ...
  if ! lsusb | grep -Eiq 'fingerprint|synaptics|goodix'; then
    ...
    if (lsusb | grep -q 2808:a658); then 
        echo -e "\e[31m\nInstalling specific driver for VivoBook fingerprint scanner.\e[0m"
        yay -S --noconfirm --needed libfprint-ft9366
    fi
...
EOF
    ...
    echo -e "\e[32m\nLet's setup your left index finger as the first fingerprint.\nKeep moving the finger around on sensor until the process completes.\n\e[0m"
    sudo fprintd-enroll -f left-index-finger $USER


    echo -e "\e[32m\nNow let's verify that it's working correctly.\e[0m\n"

    if fprintd-verify -f right-index-finger $USER && fprintd-verify -f right-index-finger $USER ; then
