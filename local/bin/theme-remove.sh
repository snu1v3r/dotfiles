...
if [ -z "$1" ]; then
  mapfile -t extra_themes < <(find ~/.local/share/themes/dynamic -mindepth 1 -maxdepth 1 -type d ! -xtype l -printf '%f\n')
...
if [ "$(readlink -f "$CURRENT_DIR")" = "$(readlink -f "$THEME_PATH")" ]; then
  ~/.local/bin/theme-next.sh
fi
